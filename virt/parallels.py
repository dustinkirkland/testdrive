...
class Parallels:
	...
	def validate_virt(self):
		if commands.getstatusoutput("prlctl list %s | grep -qsv \"UUID\"" % self.td.VBOX_NAME)[0] == 0:
			self.run_or_die("prlctl delete %s" % self.td.VBOX_NAME)
	...
	def setup_virt(self):
		...
		self.run_or_die("prlctl create %s --ostype linux --distribution ubuntu" % self.td.VBOX_NAME)
		self.run_or_die("prlctl set %s --memsize %s" % (self.td.VBOX_NAME, self.td.MEM))
		self.run_or_die("prlctl set %s --device-del hdd0" % self.td.VBOX_NAME)
		self.run_or_die("prlctl set %s --device-add hdd --type expand --size %s --iface scsi --position 0:0" % (self.td.VBOX_NAME, self.td.DISK_SIZE))
		self.run_or_die("prlctl set %s --device-set cdrom0 --image %s" % (self.td.VBOX_NAME, self.td.PATH_TO_ISO))
