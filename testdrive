...
if PROTO == "rsync":
	...
	if commands.getstatusoutput("which zsync")[0] == 0:
		if os.system("cd %s/iso && zsync %s.zsync" % (CACHE, ISO_URL))[0] != 0:
...
if VIRT == "kvm":
	...
	if os.path.exists(DISK_FILE):
		os.unlink(DISK_FILE)
	run_or_die("sed -i \":HardDisk.*%s:d\" %s/.VirtualBox/VirtualBox.xml" % (HOME, DISK_FILE))
	info("Creating disk image...")
	run_or_die("VBoxManage createhd --filename %s --size %s" % (DISK_FILE, DISK_SIZE))
	if vboxversion == "3.0":
		run_or_die("VBoxManage modifyvm %s --hda none" % VBOX_NAME)
		...
		run_or_die("VBoxManage storageattach %s --storagectl \"IDE Controller\" --port 0 --device 0 --type hdd --medium none" % VBOX_NAME)
		run_or_die("VBoxManage storageattach %s --storagectl \"IDE Controller\" --port 0 --device 1 --type dvddrive --medium none" % VBOX_NAME)
	info("Creating the Virtual Machine...")
	run_or_die("VBoxManage unregistervm %s --delete" % VBOX_NAME)
	...
	if vboxversion == "3.0":
		...
		run_or_die("VBoxManage storageattach %s --storagectl \"IDE Controlleri\" --port 0 --device 1 --type dvddrive --medium %s" % (VBOX_NAME, PATH_TO_ISO))
		run_or_die("VBoxManage startvm %s" % VBOX_NAME)
...
if os.path.exists(DISK_FILE) and os.path.getsize(DISK_FILE) == 262144:
	if md5sum(DISK_FILE) == "1da7553f642332ec9fb58a6094d2c8ef":
		info("Cleaning up empty disk image [%s]..." % DISK_FILE)
		os.unlink(DISK_FILE)
elif os.path.exists(DISK_FILE) and os.path.getsize(DISK_FILE) == 0:
	os.unlink(DISK_FILE)
else:
	# Otherwise, remind about cache cleanup
	info("You may wish to clean up the cache directory...")
	print("      %s" % CACHE)
	run_or_die("ls -HhalF %s/iso %s/img" % (CACHE, CACHE))
	run_or_die("du -sh --apparent-size %s 2>/dev/null || du -sh %s" % (CACHE, CACHE))
print("\n")
